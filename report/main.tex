...
\begin{center}
  ...
  {\normalsize MPhil Computer Engineering, KNUST \quad|\quad PG4346325}\\[4pt]
  {\small\textit{COE 562 --- Engineering Systems Design and Modelling,
  Final Modelling Assessment (Problem 13)}}\\[6pt]
  {\small August 2026}
\end{center}
\rule{\textwidth}{0.4pt}
\vspace{4mm}
...
\midrule
E2E & --- & \textbf{Unstable} && --- & \textbf{47.92} & Stable \\
\bottomrule
\end{tabular}
\end{table}
...
\]
The implementation gives $W(\lambda = 1) = \SI{24.02}{ms}$. \checkmark
...
\begin{itemize}
  \item $\rho = \lambda\,[\text{req/s}] / \mu\,[\text{req/s}]$ --- dimensionless. \checkmark
  \item $W = 1/(\mu(1-\rho))$ --- units $[\text{s/req}] = [\text{s}]$. \checkmark
  \item $L = \lambda W$ --- units $[\text{req/s}] \cdot [\text{s}] = [\text{req}]$. \checkmark
  \item $\alpha_i t$ in $e^{-\alpha_i t}$ --- units $[\text{ms}^{-1}]\cdot[\text{ms}]$ = dimensionless. \checkmark
\end{itemize}
...
\]
Algebraic substitution confirms consistency with~\eqref{eq:mm1_L}. \checkmark
...
AI tools --- including ChatGPT (for brainstorming and concept review), Grammarly
(for proofreading), and GitHub Copilot (for debugging assistance) --- were used
during the preparation of this work. Their use was limited to supporting study,
generating draft explanations for review, and identifying syntax errors in code.
All modelling decisions, derivations, analysis, and conclusions are the author's own.
